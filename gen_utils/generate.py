...
from idf_dict import get_idf_dict
from controll import langevin_fn_BertScore
...
def main(config):
    ...
    if config.load_from_ema:
        eval_model_path = os.path.join(
            config.exp.dir, 'model', f'{config.exp.name}_ema_{config.ema_rate}_checkpoint-{config.load_step}')
        print("config.exp.dir :", config.exp.dir)
    ...
    if config.model.guide_model == 'BertScore':
        src = []
        if config.data.name =='cnn_dm':
            test_src_path = os.path.join('data/cnn_dm/', 'test.src')
        else:
            test_src_path = os.path.join('data/xsum/', 'test.src')
        with open(test_src_path, "r", encoding="utf-8") as ifile:
            for line in tqdm(ifile):
                line = line.strip()
                text = line
                src.append(text)
        idf_dict = get_idf_dict(src, tokenizer)
        langevin_fn = partial(langevin_fn_BertScore, model, emb_model.cuda(), idf_dict)
    else:
        langevin_fn = None
        ...
        for i in range(config.num_samples):
            ...
            for _, batch in enumerate(tqdm(dev_dataloader)):
                ...
                sample = sample_fn(
                    ...
                    top_p=-1.0,
                    langevin_fn = langevin_fn,  # add from JW
                )
